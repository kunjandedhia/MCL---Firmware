...
/*	#define DL_DIAG_PACKET_SIZE 							(0x0000000E)*//*14 bytes Packet size of one log change23*/
/*	#define DL_DIAG_PACKET_SIZE 							(0x0000000F)*//*15 bytes Packet size of one log change49*/
	#define DL_DIAG_PACKET_SIZE 							(0x00000010)/*16 bytes Packet size of one log change54*/	
/*	#define DL_DIAG_DATABYTE_SIZE 						(0x00000006)*/ /*6 bytes Data byte size of one log*/
	#define DL_DIAG_DATABYTE_SIZE 					(0x00000007) /*7 bytes Data byte size of one log change54*/
  ...
  #define DL_STA_SEG_END										(0x00005400) /* 20K NVSRAM1 Locations*/
	...
	/*60 = 29 STATUS + 7TIMESTAMP change23(RESERVED 24 BYTES CAN BE INCREASED)*/
//	#define DL_STA_PACKET_SIZE 								(0x0000003E) 
//	#define DL_STA_PACKET_SIZE 								(0x0000003F) /*change49*/
		#define DL_STA_PACKET_SIZE 								(0x0000004B) /*change62*/
...
//	#define DL_FLT_PACKET_SIZE 								(0x0000000D) /*change23 MAX PACKET 	SIZE 12*/
	
	#define DL_FLT_PACKET_SIZE 								(0x0000000E) /*change49 MAX PACKET 	SIZE 12*/
	 /*change35 change to 5 refer Datalogging ver1.3*/
	#define DL_FLT_DATABYTE_SIZE 							(0x00000005) /*MAX DATABYTE SIZE 4 */
  ...
  #define DL_PRD_RAM2_END										(0x000FFFFF) /*LAST ADDRESS OF NVSRAM2*/
	...
	/*#define DL_PRD_PACKET_SIZE 								(0x00000047)change23*/ 
	//#define DL_PRD_PACKET_SIZE 								(0x00000049) 
	#define DL_PRD_PACKET_SIZE 								(0x0000004A)  /*change49*/
...
/* Reset BASE to zeroth location of NVSRAM2*/
	...
	#endif
	#define WARNTIMEINTERVAL								(uint8_t)01
	#define MAXWARNSEND											(uint8_t)03
...
{
	uint8_t bFault_Log_SVF:1;				/* Spray valve fault error*/
	uint8_t bFault_Log_GPVF:1;			/* Gun Position valve fault error*/
	uint8_t bFault_Log_ABVF:1;			/* AB valve fault error*/		
	uint8_t bFault_Log_FVF:1;				/* Flush valve fault error*/
	uint8_t bFault_Log_MFVF:1;			/* Motor Forward valve fault warning*/
	uint8_t bFault_Log_MBVF:1;			/* Motor Backward valve fault warning */
	uint8_t bFault_Log_STVF:1;			/* Stall valve fault error */
	uint8_t bFault_Log_RVF:1;			  /* Recirculation valve fault error change54 */
	uint8_t bFault_Log_IBLTF:1;			/* Inline Base temperature low threshold fault warning change35 */
	uint8_t bFault_Log_IBHTF:1;			/* Inline Base temperature High threshold fault change35 */
	uint8_t bFault_Log_IHLTF:1;			/* Inline Hard temperature low threshold fault  warning change35*/		
	uint8_t bFault_Log_IHHTF:1;			/* Inline Hard temperature high threshold fault change35*/	
	uint8_t bFault_Log_ALTF:1;			/* Antifreeze temperature low threshold fault warning change35 */
	uint8_t bFault_Log_AHTF:1;			/* Antifreeze temperature high threshold fault change35 */
	uint8_t bFault_Log_BRLLTHF:1;		/* Base Return Line temperature low threshold fault  warning change35 */
	uint8_t bFault_Log_BRLHTHF:1;		/* Base Return Line temperature high threshold fault change35 */
	uint8_t bFault_Log_PG1LTF:1;		/* Pressure Gauge1 Low threshold fault warning change35 */
	uint8_t bFault_Log_PG1HTF:1;		/* Pressure Gauge1 high threshold fault change35 */
	uint8_t bFault_Log_PG2LTF:1;		/* Pressure Gauge2 Low threshold fault  warning change35*/
	uint8_t bFault_Log_PG2HTF:1;		/* Pressure Gauge2 high threshold fault change35*/
	uint8_t bFault_Log_PG3LTF:1;		/* Pressure Gauge3 Low threshold fault  warning change35*/
	uint8_t bFault_Log_PG3HTF:1;		/* Pressure Gauge3 high threshold fault change35*/
	uint8_t bFault_Log_LLBLTF:1;		/* Base Liquid low Level threshold fault warning change35*/
	uint8_t bFault_Log_LLBHTF:1;		/* Base Liquid high Level threshold fault warning change35*/
	uint8_t bFault_Log_LLHLTF:1;		/* Hard Liquid low Level threshold fault warning change35*/
	uint8_t bFault_Log_LLHHTF:1;		/* Hard Liquid high Level threshold fault warning change35*/
	uint8_t bFault_Log_PTF:1;				/* Non-Contact Pipe temperature out of range fault warning */		
	uint8_t bFault_Log_IBHF:1;			/* Inline Base Heater fault warning */
	uint8_t bFault_Log_IHHF:1;			/* Inline Hard Heater fault warning */
	uint8_t bFault_Log_AHF:1;				/* Antifreeze Heater fault warning*/
	uint8_t bFault_Log_HHF:1;				/* Hardener Heater fault warning*/
	uint8_t bFault_Log_SGMF:1;			/* Spray Gun Motion fault warning */
	uint8_t bFault_Log_LTF:1;				/* Linear Transducer fault error */
	uint8_t bFault_Log_DCLSTF:1;		/* DCLS time fault error */
	uint8_t bFault_Log_HLSTF:1;			/* HLS time fault error */
	uint8_t bFault_Log_HMIHAF:1;		/* HMI heartbeat ack fault error*/
	uint8_t bFault_Log_OPMMHAF:1;		/* OPMM heartbeat ack fault  error*/
	uint8_t bFault_Log_VCMHAF:1;		/* VCM heartbeat ack fault error*/
	uint8_t bFault_Log_HCMHAF:1;		/* HCM heartbeat ack fault error */
}FL_LOG_TypeDef;
...
/******************SCU Peripharals Diagnostic Flags********/
	...
	uint8_t bDiag_Log_NVSRAM1:1;	/* NVSRAM1 Diagnostic status change54 */
	uint8_t bDiag_Log_NVSRAM2:1;	/* NVSRAM2 Diagnostic status change54 */
...
/******************VCM Peripharals Diagnostic Flags********/
	...
	uint8_t bDiag_Log_RVC:1;		/* Recirculation Valve connected change54 */
	uint8_t bDiag_Log_RV:1;			/* Recirculation valve status change54*/
...
extern uint16_t uiStallcommandsendCnt; /* Stall command send counter change7 */
extern uint16_t uiJobnameCnt; /*Counter for Jobname change78*/
extern uint16_t uiJobIDCnt; /*Counter for Jobname change78*/
extern uint16_t uiSupervisornameCnt; /*Counter for Jobname change78*/
/*ARRAYS and VARIABLES FOR DATA_LOGGING*/
extern uint8_t ucStatusByte; /* STATUS Log Byte contains bitwise logs*/
extern uint8_t rgucStatusLog[DL_STA_PACKET_SIZE]; /* Array For DATA LOGGING of STATUS LOG */
extern uint8_t rgucPeriodicLog[DL_PRD_PACKET_SIZE]; /* change15 Array For DATA LOGGING of PERIODIC LOG */
/*********************************************************************************/
 extern DATALOG_TX_t stWifiLog;
 extern DATALOG_TX_t stEthernetLog;
 /* ----------------------------------------------------------------------------
 *                           EXTERNAL FUNCTIONS
 * ----------------------------------------------------------------------------
 */
...
//	extern void	fnWifiSend_Fault_log(void);
	...
	extern void fnDataLogging(uint8_t *pucString,DATALOG_TX_t *pstLog);
/* ----------------------------------------------------------------------------
