...
#ifdef USE_Delay
 // #include "config.h"    
extern void fnDelay_ms(uint32_t nCount);	 /* Header file where the Delay function prototype is exported */
  #define _eth_delay_    fnDelay_ms   /* User can provide more timing precise _eth_delay_ function 
...
/* These values are relatives to KSZ8081 PHY and change from PHY to another,
   so the user have to update this value depending on the used external PHY */   

/* The KSZ8081 PHY status register  */
#define PHY_SR                 ((uint16_t)0x01) /* PHY status register Offset */
#define PHY_SPEED_STATUS       ((uint16_t)0x0002)//(1 << 13))//0x0002) /* PHY Speed mask */
#define PHY_DUPLEX_STATUS      ((uint16_t)0x0004)//(1 << 8))//0x0004) /* PHY Duplex mask */
#define PHY_LINK_STATUS        ((uint16_t)(1 << 2))//0x2000) /* PHY link status interrupt mask */
